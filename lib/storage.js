...
console.log('Checking for Upstash environment variables...');
...
try {
  if (process.env.UPSTASH_REDIS_REST_URL && process.env.UPSTASH_REDIS_REST_TOKEN) {
    console.log('Initializing Redis via UPSTASH_REDIS_REST_URL');
    redis = Redis.fromEnv();
  } else if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
    console.log('Initializing Redis via KV_REST_API_URL');
    redis = new Redis({
      url: process.env.KV_REST_API_URL,
      token: process.env.KV_REST_API_TOKEN,
    });
  } else {
    console.log('No Redis credentials found. Falling back to local storage.');
  }
} catch (error) {
